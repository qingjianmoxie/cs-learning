...
LeetCode 上至少有 9 道题目可以用此方法高效解决。但是有几道题目虽不难但太复杂，所以本文只选择点赞最高，较为经典的，最能够讲明白的三道题来讲解。第一题为了让读者掌握算法模板，篇幅相对长，后两题就基本秒杀了。
...
[76_最小覆盖子串](../LeetCode/76_minimum-window-substring.md)
...
[438_找到字符串中所有字母异位词(medium)](../LeetCode/438_find-all-anagrams-in-a-string.md)

实际上，只要把上一道题的代码改中更新 res 部分的代码稍加修改就成了这道题的解：
...
因为这道题和上一道的场景类似，也需要 window 中包含串 p 的所有字符，但上一道题要找长度最短的子串，这道题要找长度相同的子串。

### 三、无重复字符的最长子串

[3_无重复字符的最长子串(medium)](../LeetCode/3_longest-substring-without-repeating-characters.md)
