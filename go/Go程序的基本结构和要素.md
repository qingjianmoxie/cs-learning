...
Go 的标准库包含了大量的包（如：fmt 和 os），但是你也可以创建自己的包（详见[包（package）](./包.md)）。
...
当标识符（包括常量、变量、类型、函数名、结构字段等等）以一个大写字母开头，如：Group1，则可以被外部包的代码所使用（客户端程序需要先导入这个包），这被称为导出（像面向对象语言中的 public）；标识符如果以小写字母开头，则对包外是不可见的，但是他们在整个包的内部是可见并且可用的（像面向对象语言中的 private ）。
...
右大括号 `}` 需要被放在紧接着函数体的下一行。如果你的函数非常简短，你也可以将`{}`和函数体放在同一行：
...
go的注释和c相同, 单行注释使用 `//`, 多行注释(也叫块注释)以 `/*` 开头，并以 `*/` 结尾，且不可以嵌套使用. 多行注释一般用于包的文档描述或注释成块的代码片段。
...
示例： 函数 Atoi ([strings 和 strconv 包](./strings和strconv包.md))：`func Atoi(s string) (i int, err error)`
...
使用 type 关键字可以定义你自己的类型，你可能想要定义一个[结构体](./结构体定义.md)，但是也可以定义一个已经存在的类型的别名，如：
...
这里我们可以看到 int 是变量 a 的底层类型，这也使得它们之间存在相互转换的可能([6 类型转换](##6-类型转换))。
...
在使用 `import` 导入包之后定义或声明 0 个或多个常量（const）、变量（var）和类型（type），这些对象的作用域都是全局的（在本包范围内），所以可以被本包中所有的函数调用。
...
- [目录](README.md)
- 上一节：[文件名、关键字与标识符](./文件名,关键字与标识符.md)
- 下一节：[常量](./常量.md)
