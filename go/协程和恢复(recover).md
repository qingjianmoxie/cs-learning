...
一个用到 `recover` 的程序（参见[从panic中恢复(Recover)](./从panic中恢复(Recover).md)）停掉了服务器内部一个失败的协程而不影响其他协程的工作。
...
- [目录](README.md)
- 上一节：[通道，超时和计时器(Ticker)](./通道，超时和计时器(Ticker).md)
- 下一节：[对比新旧模型：任务和工作](./对比新旧模型：任务和工作.md)
