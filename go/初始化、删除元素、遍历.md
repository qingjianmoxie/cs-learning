...
示例 <span id="make_maps.go">make_maps.go</span>
...
和数组不同，map 可以根据新增的键值对动态的伸缩，因此它不存在固定长度或者最大限制。但是你也可以选择标明 map 的初始容量 `capacity`，就像这样：`make(map[keytype]valuetype, cap)`。例如：

```go
map2 := make(map[string]float32, 100)
```

当 map 增长到容量上限的时候，如果再增加新的键值对，map 的大小会自动加 1。所以出于性能的考虑，对于大的 map 或者会快速扩张的 map，即使只是大概知道容量，也最好先标明。
...
在例子[make_maps.go](#make_maps.go)中，我们已经见过可以使用 `val1 = map1[key1]` 的方法获取 key1 对应的值 val1。如果 map 中不存在 key1，val1 就是一个值类型的空值。
...
- [目录](README.md)
- 上一节：[Map](./Map.md)
- 下一节：[map类型的切片、排序、键值对调](./map类型的切片、排序、键值对调.md)
