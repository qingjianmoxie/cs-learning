...
从不同的并发执行的协程中获取值可以通过关键字`select`来完成，它和[`switch`控制语句](./switch结构.md)非常相似, 也被称作通信开关；它的行为像是“你准备好了吗”的轮询机制；`select`监听进入通道的数据，也可以是用通道发送值的时候。
...
`default` 语句是可选的; 与普通的switch不一样的是，fallthrough是不允许的。在任何一个 case 中执行 `break` 或者 `return`，select 就结束了。
...
- [目录](README.md)
- 上一节：[通道的同步：关闭通道-测试阻塞的通道](./通道的同步：关闭通道-测试阻塞的通道.md)
- 下一节：[通道，超时和计时器(Ticker)](./通道，超时和计时器(Ticker).md)
