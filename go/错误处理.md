...
错误值用来表示异常状态；我们可以在 [测试多返回值函数的错误](./测试多返回值函数的错误.md) 中看到它的标准用法。处理文件操作的例子可以在 [读写数据](./读写数据.md) 章节找到；我们将在 [网络、模版与网页应用](./网络、模版与网页应用.md) 章节看到网络操作的例子。errors 包中有一个 errorString 结构体实现了 error 接口。当程序处于错误状态时可以用 `os.Exit(1)` 来中止运行。
...
由于 `fmt.Printf` 会自动调用 `String()` 方法 （参见 [类型的 String() 方法和格式化描述符](./类型的String()方法和格式化描述符.md)），所以错误信息 “Error: math - square root of negative number” 会打印出来。通常（错误信息）都会有像 “Error:” 这样的前缀，所以你的错误信息不要以大写字母开头。
...
- [目录](README.md)
- 上一节：[错误处理与测试](./错误处理与测试.md)
- 下一节：[运行时异常和panic](./运行时异常和panic.md)
