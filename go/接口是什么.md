...
与大多数面向对象编程语言不同的是，在 Go 语言中接口可以有值，一个接口类型的变量或一个 **接口值** ：`var ai Namer`，`ai` 是一个多字（multiword）数据结构，它的值是 `nil`。它本质上是一个指针，虽然不完全是一回事。指向接口值的指针是非法的，它们不仅一点用也没有，还会导致代码错误。
...
有的时候，接口是另外一个意思：从某个类型的角度来看，它的接口指的是：它的所有导出方法，只不过没有显式地为这些导出方法额外定一个接口而已。
...
- [目录](README.md)
- 上一节：[接口（Interfaces）与反射（reflection）](./接口与反射.md)
- 下一节：[接口嵌套接口](./接口嵌套接口.md)
