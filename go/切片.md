...
如果某个人写：`var slice1 []type = arr1[:]` 那么 slice1 就等于完整的 arr1 数组（这种表示方式实际上是 `arr1[0:len(arr1)]` 的一种缩写）。另外一种表述方式是：`slice1 = &arr1`。
...
- new(T) 为每个新的类型T分配一片内存，初始化为 0 并且返回类型为\*T的内存地址：这种方法 **返回一个指向类型为 T，值为 0 的地址的指针**，它适用于值类型如数组和[结构体](./结构(struct)与方法(method).md)；它相当于 `&T{}`。
...
bytes 包和[字符串包](./strings和strconv包.md)十分类似。而且它还包含一个十分有用的类型 Buffer:
...
- [目录](README.md)
- 上一节：[声明和初始化](./声明和初始化.md)
- 下一节：[For-range 结构](./For-range结构.md)
