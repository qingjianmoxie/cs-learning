...
在[搜索及排序切片和数组](./字符串、数组和切片的应用.md##6-搜索及排序切片和数组)中我们看到了能被搜索和排序的 `int` 数组、`float` 数组以及 `string` 数组，那么对于其他类型的数组呢，是不是我们必须得自己编程实现它们？
...
仿照[第一个例子：使用Sorter接口排序](./第一个例子：使用Sorter接口排序)章节开发的 `Sorter` 接口，创建一个 `Miner` 接口并实现一些必要的操作。函数 `Min` 接受一个 `Miner` 类型变量的集合，然后计算并返回集合中最小的元素。
...
- [目录](README.md)
- 上一节：[第二个例子：读和写](./第二个例子：读和写.md)
- 下一节：[对结构进行反射](./对结构进行反射.md)
