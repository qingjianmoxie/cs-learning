...
上节演示了 Linux 下的 socket 程序，这节来看一下 Windows 下的 socket 程序。同样，server.cc 为服务器端代码，client.cc 为客户端代码。
...
将 server.cc 和 client.cc 分别编译为 server.exe 和 client.exe

    gcc -o server.exe server.cc -lwsock32
    gcc -o client.exe client.cc -lwsock32

先运行 server.exe，再运行 client.exe，输出结果为：
