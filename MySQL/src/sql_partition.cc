...
/*
  ...
  (Much of the code in this file has been split into partition_info.cc and
   ...
   这个文件是MySQL5.1版本中引入的与分区相关的全部功能的容器。
   它包含所有支持分区的handlers使用的功能，例如partition handler本身和NDB handler。
   （此文件中的大部分代码被拆分在partition_info.cc和头文件partition_info.h + partition_element.h + sql_partition.h中）
  ...
  partitioning and composite复合的 partitioning (hereafter called subpartitioning)
  ...
  第二个版本引入了range和list分区的新变体，在代码中经常称为column lists。
  它允许用户指定一组列及其连接值作为分区值。通过比较这些值的连接，可以选择合适的分区。
*/
